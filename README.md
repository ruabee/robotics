...
이를 위해 중앙 스케줄러를 중심으로 작업 관리 시스템을 설계하였으며, 이벤트가 발생하면 현재 수행 가능한 작업을 다시 선별한 뒤 이동 거리, 작업량, 완료 지연, 생산 순서, 작업 우선순위 등을 고려한 비용 함수를 계산하여 가장 적합한 로봇에게 작업을 재할당하도록 구현하였습니다.
