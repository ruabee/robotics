...
- Gazebo 시각화:
  ...
  - `/R1/scan`, `/R2/scan` 토픽으로 LiDAR 기반 장애물 감지 확인
  - `/R1/goal_pose`, `/R2/goal_pose` 토픽으로 Nav2 goal 연동 전 단계 확인
...
- 검은 긴 구조물: 컨베이어
- 노란 선: 로봇 이동 동선
- 파란 작은 박스: 현재 로봇이 운반 중인 부품
- 빨간 큰 박스: `o` 이벤트로 활성화되는 중앙 통로 장애물
...
여기에 `1`, `2`, `3`, `o`, `c`, `r` 중 하나를 입력하고 Enter를 누르면 Gazebo 시뮬레이션으로 이벤트가 전달됩니다.
...
LiDAR 감지 결과는 다음처럼 확인합니다.

```bash
source /opt/ros/humble/setup.bash
source /home/ruabee/Desktop/robotics/install/setup.bash
ros2 topic echo /R1/scan --once
```

Nav2로 넘길 목표 위치 형식은 다음 토픽으로 확인합니다.

```bash
source /opt/ros/humble/setup.bash
source /home/ruabee/Desktop/robotics/install/setup.bash
ros2 topic echo /R1/goal_pose
```
...
2 Enter: V2 부품 지연
3 Enter: 긴급 작업 삽입
o Enter: 중앙 통로 장애물 활성화
c Enter: 중앙 통로 장애물 해제
r Enter: 재스케줄링
q Enter: 종료
```
